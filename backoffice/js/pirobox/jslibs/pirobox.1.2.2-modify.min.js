...
start.click();piro_close.add(caption).add(piro_next).add(piro_prev).css('visibility','hidden');});$(piro_gallery).each(function(array){var item=$(this);item.unbind();item.bind('click',function(c){c.preventDefault();piro_open(item.attr('href'));var this_url=item.attr('href');var descr=item.attr('title');var number=item.attr('rel');if(descr==""){caption.html('<p>&nbsp;<em class="number">'+number+'</em><p><a href='+this_url+' class="link_to" target="_blank" title="Open Image in a new window">ดูรูปขยาย</a></p></p>');}else{caption.html('<p>'+descr+'<em class="number">'+number+'</em><p><a href='+this_url+' class="link_to" target="_blank" title="Open Image in a new window">ดูรูปขยาย</a></p></p>');}
...
imgH+=new_img_H;imgW+=new_img_W;$(img).height(new_img_H).width(new_img_W).hide();$('.c_c div').animate({height:new_img_H+'px',width:new_img_W+'px'},opt.my_speed);var fix=imgH/w_H*2.3;if(w_H<imgH){h_fix=fix;}else{h_fix=2;}
main_cont.animate({height:(new_img_H+40)+'px',width:(new_img_W+40)+'px',marginLeft:'-'+((new_img_W)/2+20)+'px',marginTop:parseInt($(document).scrollTop())-(imgH/h_fix)},opt.my_speed,function(){$('.piro_nav,.caption').css({width:(new_img_W)+'px','margin-bottom':'10px'});$('.piro_nav').css('margin-left','-'+(new_img_W)/2+'px');var caption_height=caption.height();$('.c_c div').append(img);piro_close.css('display','block');piro_next.add(piro_prev).add(piro_close).css('visibility','visible');caption.css({'visibility':'visible','display':'block','opacity':'0.8','overflow':'hidden'});main_cont.hover(function(){caption.stop().fadeTo(200,0.8);},function(){caption.stop().fadeTo(200,0);});$(img).fadeIn(300);main_cont.removeClass('loading');if(opt.slideShow===true){piro_play.add(piro_stop).show();}else{piro_play.add(piro_stop).hide();}});}else{$(img).height(imgH).width(imgW).hide();$('.c_c div').animate({height:imgH+'px',width:imgW+'px'},opt.my_speed);var fix=imgH/w_H*2.3;if(w_H<imgH){h_fix=fix;}else{h_fix=2;}
...
var timer=setInterval(slideshow,opt.slideSpeed*1000);}function scrollImage(){if($(main_cont).is(':visible')){window.onscroll=function(){var main_cont_h=$(main_cont).height();main_cont.css({marginTop:parseInt($(this).scrollTop())-(main_cont_h/1.9)-10});var Nwin_h=$(window).height();var Nnav_h=$('.piro_prev_out').height();$('.piro_prev_out').add('.piro_next_out').css({marginTop:parseInt($(document).scrollTop())+(Nwin_h/Nnav_h-125)});}}}
