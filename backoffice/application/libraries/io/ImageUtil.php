...
function getImageRatio($file, $width, $height, $defaultImage = "images/nopic.jpg") {
...
function getImageRatioPerson($file, $width, $height, $defaultImage = "images/nopic-personal.jpg") {
...
$_tempDir = './tmp/';
function getImageCrop($file, $width, $height, $defaultImage = "images/nopic.jpg", $type) {
	$fileInfo = pathinfo ( $file );
	...
	if(@$fileInfo ['extension']){
		...
		$tmpFile = './tmp/' . md5 ( $file . 'crop' . $width . $height ) . '.' . $extension;
		if (file_exists ( $tmpFile )) {
			return $tmpFile;
		} else {
			return base_url () . "image/crop/?file={$file}&width={$width}&height={$height}&defaultImage={$defaultImage}&type={$type}";
		}
	}else{
			return base_url () . "image/crop/?file={$file}&width={$width}&height={$height}&defaultImage={$defaultImage}&type={$type}";
	}
