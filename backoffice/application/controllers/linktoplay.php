...
class Linktoplay extends CI_Controller {
    /**
     *
     * @var Array All Available tables 
     */
    public $_TABLE              = Array(
        'info'              => 'linktoplay_information',
        'file'              => 'linktoplay_attactments',
        'category'          => 'linktoplay_category'
    );
    public $data                = Array(
        '_CMD'              => 'linktoplay',
        'body'              => '',
    ...
    public function showList(){
    	...
    	$cid = 2; /* for detail data page */
    	
    	
        $this->data['showMainSearch']   = true;
        $this->data['menuName']     =  'เพิ่มทางเข้าเล่น';
        $db                         = getDBO();    
        $limit       = $this->maxRows;
        $adjacent    = 2;
    ...
    public function add(){
        ...
        $frm['cid'] = 2; /* for detail data page */
    ...
    public function setNewSeq(){
    	...
    	$db->setQuery("SELECT id,seq FROM {$this->_TABLE['info']} WHERE  status = '1' AND cid = '2' ORDER BY seq ASC ");
    	$rs = $db->loadAssocList();
