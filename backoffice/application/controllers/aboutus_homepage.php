...
class Aboutus_homepage extends CI_Controller {
    /**
     *
     * @var Array All Available tables 
     */
    public $_TABLE              = Array(
        'info'              => 'aboutus_information',
        'file'              => 'aboutus_attactments',
        'category'          => 'aboutus_category'
    );
    public $data                = Array(
        '_CMD'              => 'aboutus_homepage',
        'body'              => '',
        'menuName'          => 'Topic management'
    );
    public $maxRows             = 10;
    public $avialableTag        = '<p><a><img><span><div><table><tbody><tr><td><th><ul><li><qoute><font><ol><style><strong><em><u><i><h4><h5><h6><br><br/>'; 
    /*
     * Predined methods
     */
    ...
    public function showList(){
    	...
    	$cid = 1; /* for detail data page */
    	
    	
        $this->data['showMainSearch']   = true;
        $this->data['menuName']     =  'เพิ่มข้อมูลเกี่ยวกับเรา';
        $db                         = getDBO();    
        $limit       = $this->maxRows;
        $adjacent    = 2;
    ...
    public function add(){
        ...
        $frm['cid'] = 1; /* for detail data page */
    ...
    public function edit(){
    	...
    	$this->data['menuName'] = "แก้ไขข้อมูลการแสดงผลหน้าแรก";
    	$this->data['iconName'] = '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>';
    	
    	/* For homepage */
    	$db                     = getDBO();
    	
    	$db->setQuery(" SELECT id FROM {$this->_TABLE['info']} WHERE cid = '1' AND status = '1' ORDER BY id DESC ");
        $rshome = $db->loadAssocList();
    ...
    public function setNewSeq(){
    	...
    	$db->setQuery("SELECT id,seq FROM {$this->_TABLE['info']} WHERE  status = '1' ORDER BY seq ASC ");
    	$rs = $db->loadAssocList();
