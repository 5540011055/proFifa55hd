...
class Banner extends CI_Controller {
     ...
     */
    public $_TABLE              = Array(
        'info'              => 'banner_information',
        'file'              => 'banner_attactments',
    ...
    public function showList(){
        ...
        $this->data['menuName']     =  'เพิ่มแบนเนอร์';
        $db                         = getDBO();    
        $limit       = $this->maxRows;
        $adjacent    = 2;
       ...
       // $dropdown                   = (@$_REQUEST['cid']) ? " AND i.cid = '".$_REQUEST['cid']."' " : "";
        ...
        $query                      = " SELECT i.id,i.subject,i.seq,i.display_image,i.status,CONCAT(i.create_date,'__',i.create_by) As cbydate,
    ...
    public function getMaxSeq(){
    	...
    	$db->setQuery("SELECT max(seq) As seq FROM {$this->_TABLE['info']} WHERE status <> '2' ");
    	$rs = $db->loadAssocList();
    ...
    public  function swap_seq($result_seq,$last_db_seq){
    	...
    	$db->setQuery("SELECT id,seq FROM {$this->_TABLE['info']} WHERE  status <> '2' AND seq >= '".$result_seq."' ");
    	$rs = $db->loadAssocList();
    ...
    public  function swap_seq_update($result_seq,$last_db_seq){
    	...
    	if($result_seq < $last_db_seq){
    		...
    		$db->setQuery("SELECT id,seq FROM {$this->_TABLE['info']} WHERE  status <> '2' AND seq < '".$last_db_seq."' ");
    		$rs = $db->loadAssocList();
    ...
    public function setNewSeq(){
    	...
    	$db->setQuery("SELECT id,seq FROM {$this->_TABLE['info']} WHERE  status <> '2' ORDER BY seq ASC ");
    	$rs = $db->loadAssocList();
