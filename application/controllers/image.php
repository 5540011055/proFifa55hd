<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Image extends CI_Controller {
        ...
        public function crop(){
        	...
        	$file               = request('file','');
        	$defaultImage       = request('defaultImage', 'images/default.jpg');
        	...
        	$tmpFile            = $this->_tempDir. md5($file.'crop'.$maxWidth.$maxHeight.$percen). '.' .$extension;
        	if(!file_exists($tmpFile)){
        		...
        		if(file_exists($tmpFile)){
        			...
        			if($width>$height OR $width==$height){
        				if(!($height<$maxHeight)){
        					$y_axis = abs((((($maxWidth/$maxHeight)/($width/$height))*$height)-$height)*($percen/100));
        					#if($height<$maxHeight){ $y_axis = 0; }
        					$config['image_library']    = 'gd2';
